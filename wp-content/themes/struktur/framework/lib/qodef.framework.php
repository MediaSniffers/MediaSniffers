...
/*
   Class: StrukturSelectClassFramework
...
class StrukturSelectClassFramework {
	...
	private function __construct() {
		$this->qodeOptions         = StrukturSelectClassOptions::get_instance();
		$this->qodeMetaBoxes       = StrukturSelectClassMetaBoxes::get_instance();
		$this->qodeTaxonomyOptions = StrukturSelectClassTaxonomyOptions::get_instance();
		$this->qodeUserOptions     = StrukturSelectClassUserOptions::get_instance();
		$this->qodeDashboardOptions = StrukturSelectClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( StrukturSelectClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class StrukturSelectClassSkinManager
 *
...
class StrukturSelectClassSkinManager {
	...
	/**
	 * @see StrukturSelectClassSkinManager::setSkin()
	 */
	 ...
	 */
	private function setSkin( $skinName = STRUKTUR_SELECT_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
...
/**
 * Class StrukturSelectClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class StrukturSelectClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( struktur_select_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.qodeSCIcon = '<?php echo struktur_select_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.qodeSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'struktur' ) ?>';
		</script>
	<?php }
...
/*
   Class: StrukturSelectClassOptions
...
class StrukturSelectClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $struktur_select_global_options ) ) {
			return $struktur_select_global_options[ $key ];
...
/*
   Class: StrukturSelectClassAdminPage
   A class that initializes Select Admin Page
*/
class StrukturSelectClassAdminPage implements iStrukturSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new StrukturSelectClassFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iStrukturSelectInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: StrukturSelectClassMetaBoxes
...
/*
   Class: StrukturSelectClassMetaBox
   A class that initializes Select Meta Box
*/
class StrukturSelectClassMetaBox implements iStrukturSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $struktur_select_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: StrukturSelectClassTaxonomyOptions
   A class that initializes StrukturSelectClass Taxonomy Options
*/
class StrukturSelectClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: StrukturSelectClassTaxonomyOption
   A class that initializes StrukturSelectClass Taxonomy Option
*/
class StrukturSelectClassTaxonomyOption implements iStrukturSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new StrukturSelectClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: StrukturSelectClassUserOptions
   A class that initializes StrukturSelectClass User Options
*/
class StrukturSelectClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: StrukturSelectClassUserOption
   A class that initializes StrukturSelectClass User Option
*/
class StrukturSelectClassUserOption implements iStrukturSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new StrukturSelectClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: StrukturSelectClassDashboardOptions
   A class that initializes StrukturSelectClass Dashboard Options
*/
class StrukturSelectClassDashboardOptions {
	private static $instance;
...
/*
   Class: StrukturSelectClassDashboardOption
   A class that initializes StrukturSelectClass Dashboard Option
*/
class StrukturSelectClassDashboardOption implements iStrukturSelectInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new StrukturSelectClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'struktur_select_init_framework_variable' ) ) {
	function struktur_select_init_framework_variable() {
		global $struktur_select_global_Framework;
		
		$struktur_select_global_Framework = StrukturSelectClassFramework::get_instance();
		$qodeSkinManager                 = new StrukturSelectClassSkinManager();
		$struktur_select_global_Framework->setSkin( $qodeSkinManager->getSkin() );
	}
	
	add_action( 'struktur_select_action_before_options_map', 'struktur_select_init_framework_variable' );
}
?>