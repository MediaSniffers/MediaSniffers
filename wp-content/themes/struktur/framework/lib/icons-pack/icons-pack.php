...
include STRUKTUR_SELECT_FRAMEWORK_ICONS_ROOT_DIR . '/icons-pack-interface.php';

foreach ( glob( STRUKTUR_SELECT_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( struktur_select_is_customizer_item_enabled( $module_load, 'struktur_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'struktur_select_get_icon_pack_collections' ) ) {
	function struktur_select_get_icon_pack_collections() {
		$options = apply_filters( 'struktur_select_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'struktur_select_get_icon_pack_options' ) ) {
	function struktur_select_get_icon_pack_options() {
		$options = apply_filters( 'struktur_select_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
}

if ( ! function_exists( 'struktur_select_get_icon_pack_social_options' ) ) {
	function struktur_select_get_icon_pack_social_options() {
		$options = apply_filters( 'struktur_select_filter_add_icon_pack_into_social_options', $options = array() );
...
/*
 * Class: StrukturSelectClassIconCollections
 */
class StrukturSelectClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = struktur_select_get_icon_pack_collections();
		$this->iconPackOptions       = struktur_select_get_icon_pack_options();
		$this->iconPackSocialOptions = struktur_select_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = struktur_select_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'struktur' ),
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'        => 'dropdown',
					'heading'     => esc_html__( 'Icon', 'struktur' ),
    ...
    public function getElementorParamsArray($control_object, $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false){
        ...
        $control_object->add_control(
            'icon_pack', [
                'label' => esc_html__( 'Icon Pack', 'struktur' ),
            ...
            foreach ($this->iconCollections as $key => $collection) {
                $control_object->add_control(
                    $iconCollectionPrefix . $collection->param, [
                        'label' => esc_html__( 'Icon', 'struktur' ),
                        'type' => \Elementor\Controls_Manager::SELECT,
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'struktur' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'struktur' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'struktur' ), $n, $collection->title ),
						'options' => array_flip( $collection->getSocialIconsArrayVC() )
					);
				}
			}
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'struktur' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'struktur' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'struktur' )       => 'fa-lg',
			esc_html__( 'Small', 'struktur' )      => 'fa-2x',
			esc_html__( 'Medium', 'struktur' )     => 'fa-3x',
			esc_html__( 'Large', 'struktur' )      => 'fa-4x',
			esc_html__( 'Very Large', 'struktur' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iStrukturSelectInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'struktur' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'struktur' ) ] = '';
	...
	public function enqueueStyles() {
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $collection_key => $collection_obj ) {
				wp_enqueue_style( 'struktur-select-' . $collection_key, $collection_obj->styleUrl );
			}
		}
	}
