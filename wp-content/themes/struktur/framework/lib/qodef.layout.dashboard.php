...
/*
   Class: StrukturSelectClassDashboardForm
   A class that initializes Select Dashboard Container
*/

class StrukturSelectClassDashboardForm implements iStrukturSelectInterfaceLayoutNode, iStrukturSelectInterfaceRender {
	public $children;
	public $name;
	public $form_id;
	public $form_method;
...
/*
   Class: StrukturSelectClassDashboardGroup
   A class that initializes StrukturSelectClass Group Field
*/
class StrukturSelectClassDashboardGroup implements iStrukturSelectInterfaceLayoutNode, iStrukturSelectInterfaceRender {
	public $children;
	public $name;
	public $title;
	public $description;
...
/*
   Class: StrukturSelectClassDashboardTitle
   A class that initializes Dashboard Title
*/
class StrukturSelectClassDashboardTitle implements iStrukturSelectInterfaceRender {
	private $name;
	private $title;
	private $args = array();
...
/*
   Class: StrukturSelectClassDashboardField
   A class that initializes StrukturSelectClass Front Field
*/
class StrukturSelectClassDashboardField implements iStrukturSelectInterfaceRender {
	private $type;
	private $name;
	private $label;
	private $description;
...
abstract class StrukturSelectClassDashboardFieldType {
	abstract public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = "" );
}

class StrukturSelectClassDashboardFieldText extends StrukturSelectClassDashboardFieldType {
...
class StrukturSelectClassDashboardFieldTextArea extends StrukturSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class StrukturSelectClassDashboardFieldImage extends StrukturSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
			...
			<div class="qodef-dashboard-gallery-holder">
				<ul class="qodef-dashboard-gallery-images-holder">
					<?php if ( isset( $value_html ) ) {
						echo struktur_select_get_module_part( $value_html );
					} ?>
				</ul>
				<div class="qodef-dashboard-gallery-uploader">
					<?php
					if ( struktur_membership_theme_installed() ) {
						echo struktur_select_get_button_html( array(
							'text'         => esc_html__( 'Upload', 'struktur' ),
							'custom_class' => 'qodef-dashboard-gallery-upload'
						) );
					} else {
						echo '<a itemprop="url" href="#" class="qodef-btn qodef-btn-medium qodef-btn-solid qodef-dashboard-gallery-upload"><span class="qodef-btn-text">' . esc_html__( 'Upload', 'struktur' ) . '</span></a>';
					} ?>
					...
					<?php if ( $value !== '' && $value !== false ) { ?>
						<button class="qodef-btn qodef-btn-solid qodef-dashboard-remove-image"><?php esc_html_e( 'Remove Media', 'struktur' ); ?></button>
					<?php } ?>
				</div>
			</div>
		</div>
...
class StrukturSelectClassDashboardFieldGallery extends StrukturSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class StrukturSelectClassDashboardFieldSelect extends StrukturSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$class = '';
...
class StrukturSelectClassDashboardFieldDate extends StrukturSelectClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class StrukturSelectClassDashboardFieldIcon extends StrukturSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$options           = struktur_select_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = struktur_select_icon_collections()->getIconCollectionsKeys();
				...
				<div class="qodef-dashboard-item">
					...
					<div class="qodef-dashboard-icon-holder">
						...
						<?php foreach ( $icons_collections as $icons_collection ) {
							$icons_param = struktur_select_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
							...
							<div class="qodef-icon-collection-holder <?php echo esc_attr( $field_class ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
								<select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" id="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>">
									<?php
									$icons       = struktur_select_icon_collections()->getIconCollection( $icons_collection );
...
class StrukturSelectClassDashboardFieldColor extends StrukturSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		if ( ! empty( $repeat ) && array_key_exists( 'name', $repeat ) && array_key_exists( 'index', $repeat ) ) {
			$id   = $name . '-' . $repeat['index'];
			$name = $repeat['name'] . '[' . $repeat['index'] . '][' . $name . ']';
...
class StrukturSelectClassDashboardFieldCheckBoxGroup extends StrukturSelectClassDashboardFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
...
class StrukturSelectClassDashboardFieldAddress extends StrukturSelectClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		$col_width = 12;
		if ( isset( $args["col_width"] ) ) {
			$col_width = $args["col_width"];
			...
			<div class="qodef-dashboard-field-row">
				<div class="qodef-dashboard-item col-lg-<?php echo esc_attr( $col_width ); ?>">
					<div class="qodef-dashboard-input-holder">
						...
						<a class="qodef-reset-marker qodef-hide-field" href="#"><?php esc_html_e( 'Reset Marker', 'struktur' ); ?></a>
...
class StrukturSelectClassDashboardRepeater implements iStrukturSelectInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false, $value = array() ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'struktur' );
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="qodef-dashboard-repeater-table-heading">
					<div class="qodef-dashboard-repeater-fields-holder">
						<div class="qodef-dashboard-repeater-table-cell qodef-dashboard-repeater-sort"><?php esc_html_e( 'Order', 'struktur' ) ?></div>
						...
						<div class="qodef-dashboard-repeater-table-cell qodef-dashboard-repeater-remove"><?php esc_html_e( 'Remove', 'struktur' ) ?></div>
					</div>
				</div>
			<?php } ?>
...
class StrukturSelectClassDashboardFieldFactory {
	public function render( $field_type, $name, $label = "", $description = "", $options = array(), $args = array(), $value = '', $repeat = array() ) {
		switch ( strtolower( $field_type ) ) {
			case 'text':
				$field = new StrukturSelectClassDashboardFieldText();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'textarea':
				$field = new StrukturSelectClassDashboardFieldTextArea();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'date':
				$field = new StrukturSelectClassDashboardFieldDate();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'image':
				$field = new StrukturSelectClassDashboardFieldImage();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'gallery':
				$field = new StrukturSelectClassDashboardFieldGallery();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'select':
				$field = new StrukturSelectClassDashboardFieldSelect();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'icon':
				$field = new StrukturSelectClassDashboardFieldIcon();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'color':
				$field = new StrukturSelectClassDashboardFieldColor();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'checkboxgroup':
				$field = new StrukturSelectClassDashboardFieldCheckBoxGroup();
				$field->render( $name, $label, $description, $options, $args, $value, $repeat );
				break;
			
			case 'address':
				$field = new StrukturSelectClassDashboardFieldAddress();
