...
if ( class_exists( 'StrukturCoreClassWidget' ) ) {
	class StrukturSelectClassAuthorInfoWidget extends StrukturCoreClassWidget {
		public function __construct() {
			parent::__construct(
				'qodef_author_info_widget',
				esc_html__( 'Struktur Author Info Widget', 'struktur' ),
				array( 'description' => esc_html__( 'Add author info element to widget areas', 'struktur' ) )
			);
		...
		protected function setParams() {
			$this->params = array(
				...
				array(
					...
					'title' => esc_html__( 'Author Username', 'struktur' )
				),
				array(
					'type'  => 'image',
					'name'  => 'big_image',
					'title' => esc_html__( 'Big Image', 'struktur' )
				),
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			<div class="widget qodef-author-info-widget <?php echo esc_attr( $extra_class ); ?>" <?php echo struktur_select_get_inline_style( $widget_styles ); ?>>
				<div class="qodef-aiw-inner">
					<a itemprop="url" class="qodef-aiw-image" href="<?php echo esc_url( get_author_posts_url( $authorID ) ); ?>">
						<?php echo struktur_select_kses_img( get_avatar( $authorID, 138 ) ); ?>
					</a>
					<?php if ( $author_info !== "" ) { ?>
						<h4 class="qodef-aiw-title"><?php esc_html_e( 'About Author', 'struktur' ); ?></h4>
