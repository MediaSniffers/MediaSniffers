<?php
namespace StrukturCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'struktur' ),
					'base'                      => $this->getBase(),
					'category'                  => esc_html__( 'by STRUKTUR', 'struktur' ),
					...
					'params'                    => array(
						array(
							...
							'heading'     => esc_html__( 'Selected Product', 'struktur' ),
							'settings'    => array(
								'sortable'      => true,
								'unique_values' => true
							),
							'admin_label' => true,
							'save_always' => true,
							'description' => esc_html__( 'If you left this field empty then product ID will be of the current page', 'struktur' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'product_info_type',
							'heading'     => esc_html__( 'Product Info Type', 'struktur' ),
							'value'       => array(
								esc_html__( 'Title', 'struktur' )          => 'title',
								esc_html__( 'Featured Image', 'struktur' ) => 'featured_image',
								esc_html__( 'Category', 'struktur' )       => 'category',
								esc_html__( 'Excerpt', 'struktur' )        => 'excerpt',
								esc_html__( 'Price', 'struktur' )          => 'price',
								esc_html__( 'Rating', 'struktur' )         => 'rating',
								esc_html__( 'Add To Cart', 'struktur' )    => 'add_to_cart',
								esc_html__( 'Tag', 'struktur' )            => 'tag',
								esc_html__( 'Author', 'struktur' )         => 'author',
								esc_html__( 'Date', 'struktur' )           => 'date',
							),
							'admin_label' => true
						),
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'struktur' ),
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'title', 'category', 'excerpt', 'price', 'rating', 'tag', 'author', 'date' ) ),
							'group'      => esc_html__( 'Product Info Style', 'struktur' )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'title_tag',
							'heading'     => esc_html__( 'Title Tag', 'struktur' ),
							'value'       => array_flip( struktur_select_get_title_tag( true, array( 'p' => 'p' ) ) ),
							'description' => esc_html__( 'Set title tag for product title element', 'struktur' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Featured Image Proportions', 'struktur' ),
							'value'      => array(
								esc_html__( 'Default', 'struktur' )        => '',
								esc_html__( 'Original', 'struktur' )       => 'full',
								esc_html__( 'Square', 'struktur' )         => 'struktur_select_image_square',
								esc_html__( 'Landscape', 'struktur' )      => 'struktur_select_image_landscape',
								esc_html__( 'Portrait', 'struktur' )       => 'struktur_select_image_portrait',
								esc_html__( 'Medium', 'struktur' )         => 'medium',
								esc_html__( 'Large', 'struktur' )          => 'large',
								esc_html__( 'Shop Single', 'struktur' )    => 'woocommerce_single',
								esc_html__( 'Shop Thumbnail', 'struktur' ) => 'woocommerce_thumbnail'
							),
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'struktur' ),
							'value'       => array_flip( struktur_select_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'struktur' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'struktur' ),
							'value'      => array(
								esc_html__( 'Default', 'struktur' ) => '',
								esc_html__( 'White', 'struktur' )   => 'white',
								esc_html__( 'Dark', 'struktur' )    => 'dark'
							),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'struktur' ),
							'value'      => array(
								esc_html__( 'Default', 'struktur' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'struktur' ) ),
							'group'      => esc_html__( 'Product Info Style', 'struktur' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="qodef-pi-title entry-title" ' . struktur_select_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="qodef-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . struktur_select_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="qodef-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . struktur_select_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="qodef-pi-excerpt"><p itemprop="description" class="qodef-pi-excerpt-item" ' . struktur_select_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="qodef-pi-price" ' . struktur_select_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="qodef-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "struktur" ), $average ) . '" ' . struktur_select_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		if ( ! $product->is_in_stock() ) {
			$button_classes = 'button ajax_add_to_cart qodef-btn qodef-btn-solid qodef-out-of-stock';
		...
		$html .= apply_filters( 'struktur_select_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="qodef-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="qodef-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . struktur_select_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="qodef-pi-author" ' . struktur_select_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="qodef-pi-date" ' . struktur_select_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
	...
	public function productIdAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				$data = array();
				$data['value'] = $value['id'];
				$data['label'] = esc_html__( 'Id', 'struktur' ) . ': ' . $value['id'] . ( ( strlen( $value['title'] ) > 0 ) ? ' - ' . esc_html__( 'Title', 'struktur' ) . ': ' . $value['title'] : '' );
				$results[] = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( ! is_wp_error( $product ) ) {
				...
				if ( ! empty( $product_title ) ) {
					$product_title_display = ' - ' . esc_html__( 'Title', 'struktur' ) . ': ' . $product_title;
				}
				
				$product_id_display = esc_html__( 'Id', 'struktur' ) . ': ' . $product_id;
