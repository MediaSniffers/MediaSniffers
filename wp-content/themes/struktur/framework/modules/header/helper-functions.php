...
if ( ! function_exists( 'struktur_select_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function struktur_select_header_skin_class( $classes ) {
		$header_style     = struktur_select_get_meta_field_intersect( 'header_style', struktur_select_get_page_id() );
		$header_style_404 = struktur_select_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'struktur_select_header_skin_class' );
}

if ( ! function_exists( 'struktur_select_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function struktur_select_sticky_header_behaviour_class( $classes ) {
		$header_behavior = struktur_select_get_meta_field_intersect( 'header_behaviour', struktur_select_get_page_id() );
	...
	add_filter( 'body_class', 'struktur_select_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'struktur_select_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function struktur_select_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = struktur_select_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'struktur_select_menu_dropdown_appearance' );
}

if ( ! function_exists( 'struktur_select_header_class' ) ) {
	/**
	 ...
	 */
	function struktur_select_header_class( $classes ) {
		...
		$disable_menu_area_shadow = struktur_select_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'qodef-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = struktur_select_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'qodef-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = struktur_select_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'qodef-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = struktur_select_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'qodef-menu-area-in-grid-border-disable';
		}
		
		if ( struktur_select_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     struktur_select_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     struktur_select_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'qodef-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = struktur_select_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'qodef-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = struktur_select_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'qodef-logo-area-in-grid-border-disable';
		}
		
		if ( struktur_select_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     struktur_select_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     struktur_select_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'qodef-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = struktur_select_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'qodef-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = struktur_select_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'struktur_select_header_class' );
}

if ( ! function_exists( 'struktur_select_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function struktur_select_header_area_style( $style ) {
		...
		$menu_area_enable_border      = get_post_meta( $page_id, 'qodef_menu_area_border_meta', true );
		...
		$menu_area_container_inner_selector = array( $class_prefix . ' .qodef-page-header .qodef-vertical-align-containers-inner' );
		...
		$menu_area_background_color_rgba = struktur_select_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		if ( $menu_area_height !== '' ) {
			$menu_area_style['height'] = struktur_select_filter_px( $menu_area_height ) . 'px !important';
		}
		...
		$menu_area_border_style = array();
		
		if ( $menu_area_enable_border !== 'no' ) {
			$header_border_color = get_post_meta( $page_id, 'qodef_menu_area_border_color_meta', true );
			
			if ( $header_border_color !== '' ) {
				$menu_area_border_style['border-bottom'] = '2px solid ' . $header_border_color;
			}
		} else {
			$menu_area_border_style['border-bottom'] = 'none';
		}
		...
		if ( $container_side_padding !== '' ) {
			...
			$current_style .= struktur_select_dynamic_css( $menu_container_selector, $menu_container_styles );
		}
		...
		$menu_area_grid_background_color_rgba = struktur_select_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( ! empty( $menu_area_grid_background_color_rgba ) ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= struktur_select_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= struktur_select_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		$current_style .= struktur_select_dynamic_css( $menu_area_container_inner_selector, $menu_area_border_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = struktur_select_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .qodef-page-header .qodef-drop-down .second' );
		
		$current_style .= struktur_select_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = struktur_select_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		if ( $logo_area_height !== '' ) {
			$logo_area_style['height'] = struktur_select_filter_px( $logo_area_height ) . 'px !important';
		}
		...
		$logo_area_grid_background_color_rgba = struktur_select_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= struktur_select_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= struktur_select_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'struktur_select_filter_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'struktur_select_filter_add_page_custom_style', 'struktur_select_header_area_style' );
}