<div class="qodef-reviews-list-info qodef-reviews-per-mark">
	<?php foreach ( $post_ratings as $rating ) { ?>
		<?php
		$average_rating     = struktur_core_post_average_rating( $rating );
		$rating_count       = $rating['count'];
		$rating_count_label = $rating_count === 1 ? esc_html__( 'Rating', 'struktur-core' ) : esc_html__( 'Ratings', 'struktur-core' );
				...
				<div class="qodef-reviews-number-wrapper">
					...
					<span class="qodef-stars-wrapper">
                        ...
                        <span class="qodef-reviews-count">
                            <?php echo esc_html__( 'Rated', 'struktur-core' ) . ' ' . $average_rating . ' ' . esc_html__( 'out of', 'struktur-core' ) . ' ' . $rating_count . ' ' . $rating_count_label; ?>
                        </span>
                    </span>
				</div>
			</div>
			<div class="qodef-grid-col-8">
				<div class="qodef-rating-percentage-wrapper">
					...
					foreach ( $rating_marks as $item => $value ) {
						...
						echo do_shortcode( '[qodef_progress_bar percent="' . esc_attr( $percentage ) . '" title="' . esc_attr( $item ) . esc_html__( ' stars', 'struktur-core' ) . '"]' );
					}
					?>
				</div>
			</div>
