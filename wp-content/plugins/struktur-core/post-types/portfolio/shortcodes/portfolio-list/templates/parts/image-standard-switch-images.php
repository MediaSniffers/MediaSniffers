...
<div class="qodef-pli-image">
    ...
    <?php if (!empty($switch_featured_image)) { ?>
        <div class="qodef-switch-image">
            <?php if ($thumb_size === 'full') { ?>
                <img itemprop="image" class="qodef-pl-hover-image" src="<?php echo esc_url($switch_featured_image); ?>" alt="<?php esc_attr_e('Portfolio Hover Featured Image', 'struktur-core'); ?>" />
            <?php } else {
                $thumb_image_size = struktur_select_get_image_size($thumb_size);
                echo struktur_select_generate_thumbnail(null, $switch_featured_image, $thumb_image_size['width'], $thumb_image_size['height']);
            } ?>
        </div>
    <?php } ?>
</div>