...
class StrukturSelectClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_struktur_core_action_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_struktur_core_action_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'struktur-core' );
		
		if ( isset( $_COOKIE[ 'qodef-like_' . $post->ID ] ) ) {
			$class = 'qodef-like liked';
			$title = esc_html__( 'You already like this!', 'struktur-core' );
		}
...
if ( ! function_exists( 'struktur_core_create_like' ) ) {
	function struktur_core_create_like() {
		StrukturSelectClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'struktur_core_create_like' );
}