...
class StrukturCoreImport {
	...
	function __construct() {
		...
		$this->importURI       = defined( 'STRUKTUR_SELECT_PROFILE_SLUG' ) ? 'http://export.' . STRUKTUR_SELECT_PROFILE_SLUG . '-themes.com/' : '';
		
		add_action( 'admin_menu', array( &$this, 'qodef_admin_import' ) );
		add_action( 'admin_menu', array( &$this, 'qodef_register_theme_settings' ), 20 ); // permission 20 is set in order to qodef_register_theme_settings be after the qodef_admin_import
	}
	...
	public function import_content( $file ) {
		ob_start();
		require_once( STRUKTUR_CORE_ABS_PATH . '/import/class.wordpress-importer.php' );
		$struktur_import = new WP_Import();
		set_time_limit( 0 );
		
		$struktur_import->fetch_attachments = $this->attachments;
		$returned_value                  = $struktur_import->import( $file );
		
		if ( is_wp_error( $returned_value ) ) {
			$this->message = esc_html__( 'An Error Occurred During Import', 'struktur-core' );
		} else {
			$this->message = esc_html__( 'Content imported successfully', 'struktur-core' );
		}
	...
	public function import_widgets( $file, $file2 ) {
		...
		foreach ( (array) $options['widgets'] as $struktur_widget_id => $struktur_widget_data ) {
			update_option( 'widget_' . $struktur_widget_id, $struktur_widget_data );
		}
		
		$this->import_sidebars_widgets( $file );
		$this->message = esc_html__( 'Widgets imported successfully', 'struktur-core' );
	}
	...
	public function import_custom_sidebars( $file ) {
		...
		$this->message = esc_html__( 'Custom sidebars imported successfully', 'struktur-core' );
	}
	
	public function import_options( $file ) {
		$options       = $this->file_options( $file );
		$result        = update_option( 'qodef_options_struktur', $options );
		$this->message = esc_html__( 'Options imported successfully', 'struktur-core' );
	}
	...
	public function import_settings_pages( $file ) {
		...
		foreach ( $pages as $struktur_page_option => $struktur_page_id ) {
			update_option( $struktur_page_option, $struktur_page_id );
		}
	}
	...
	function qodef_admin_import() {
		if ( struktur_core_theme_installed() ) {
			...
			$this->pagehook = add_submenu_page(
				...
				esc_html__( 'Struktur Options - Struktur Import', 'struktur-core' ), // The value used to populate the browser's title bar when the menu page is active
				esc_html__( 'Import', 'struktur-core' ),                     // The text of the menu in the administrator's sidebar
				'administrator',                                          // What roles are able to access the menu
                STRUKTUR_SELECT_OPTIONS_SLUG . $slug,                  // The ID used to bind submenu items to this menu
				array( $struktur_select_global_Framework->getSkin(), 'renderImport' )
			);
			
			add_action( 'admin_print_scripts-' . $this->pagehook, 'struktur_select_enqueue_admin_scripts' );
			add_action( 'admin_print_styles-' . $this->pagehook, 'struktur_select_enqueue_admin_styles' );
		}
	}
	...
	function qodef_update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( 'qodef_options_struktur' );
			$new_global_values = str_replace( $demo_url, $url, $global_options );
			
			update_option( 'qodef_options_struktur', $new_global_values );
		}
	}
	
	function qodef_import_get_demo_urls( $folder ) {
		$demo_urls  = array();
		$domain_url = defined( 'STRUKTUR_SELECT_PROFILE_SLUG' ) ? str_replace( '/', '', $folder ) . '.' . STRUKTUR_SELECT_PROFILE_SLUG . '-themes.com/' : '';
