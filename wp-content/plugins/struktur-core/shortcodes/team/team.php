<?php
namespace StrukturCore\CPT\Shortcodes\Team;
...
class Team implements lib\ShortcodeInterface {
	...
	public function vcMap() {
		...
		for ( $x = 1; $x < 6; $x ++ ) {
			$teamIconCollections = struktur_select_icon_collections()->getCollectionsWithSocialIcons();
			foreach ( $teamIconCollections as $collection_key => $collection ) {
				
				$team_social_icons_array[] = array(
					'type'       => 'dropdown',
					'heading'    => esc_html__( 'Social Icon ', 'struktur-core' ) . $x,
			...
			$team_social_icons_array[] = array(
				'type'       => 'textfield',
				'heading'    => esc_html__( 'Social Icon ', 'struktur-core' ) . $x . esc_html__( ' Link', 'struktur-core' ),
				'param_name' => 'team_social_icon_' . $x . '_link',
				'dependency' => array(
					'element' => 'team_social_icon_pack',
					'value'   => struktur_select_icon_collections()->getIconCollectionsKeys()
				)
			);
			
			$team_social_icons_array[] = array(
				'type'       => 'dropdown',
				'heading'    => esc_html__( 'Social Icon ', 'struktur-core' ) . $x . esc_html__( ' Target', 'struktur-core' ),
				'param_name' => 'team_social_icon_' . $x . '_target',
				'value'      => array(
					esc_html__( 'Same Window', 'struktur-core' ) => '_self',
					esc_html__( 'New Window', 'struktur-core' )  => '_blank'
				),
		...
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Team', 'struktur-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						array(
							array(
								...
								'value'       => array(
									esc_html__( 'Info Below Image', 'struktur-core' )    => 'info-below-image',
									esc_html__( 'Info On Image Hover', 'struktur-core' ) => 'info-on-image'
								),
								'save_always' => true
							),
							array(
							...
							array(
								...
								'heading'    => esc_html__( 'Enable Pattern Overlay', 'struktur' ),
								'value'      => array_flip( struktur_select_get_yes_no_select_array( false ) )
							),
							array(
								'type'        => 'colorpicker',
								'param_name'  => 'overlay_pattern_color',
								'heading'     => esc_html__( 'Overlay Pattern Color', 'struktur-core' ),
								'save_always' => true,
								'dependency'  => array( 'element' => 'overlay_pattern', 'value' => 'yes' )
							),
							array(
								'type'       => 'textfield',
								'param_name' => 'team_name',
								'heading'    => esc_html__( 'Name', 'struktur-core' )
							),
							array(
								'type'        => 'dropdown',
								'param_name'  => 'team_name_tag',
								'heading'     => esc_html__( 'Name Tag', 'struktur-core' ),
							...
							array(
								...
								'heading'    => esc_html__( 'Name Color', 'struktur-core' ),
								'dependency' => array( 'element' => 'team_name', 'not_empty' => true )
							),
							array(
								'type'       => 'textfield',
							...
							array(
								...
								'heading'    => esc_html__( 'Position Color', 'struktur-core' ),
							...
							array(
								...
								'heading'     => esc_html__( 'Social Icon Pack', 'struktur-core' ),
								'value'       => array_merge( array( '' => '' ), struktur_select_icon_collections()->getIconCollectionsVCExclude( 'linea_icons' ) ),
								'save_always' => true
							),
						),
	...
	public function render( $atts, $content = null ) {
		...
		for ( $x = 1; $x <= $number_of_social_icons; $x ++ ) {
			...
			foreach ( struktur_select_icon_collections()->iconCollections as $collection_key => $collection ) {
		...
		$html = struktur_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'team', '', $params );
	...
	private function getTeamSocialIcons( $params ) {
		...
		if ( $team_social_icon_pack !== '' ) {
			...
			$icon_pack                    = struktur_select_icon_collections()->getIconCollection( $team_social_icon_pack );
			...
			for ( $i = 1; $i <= $params['number_of_social_icons']; $i ++ ) {
				...
				if ( $team_social_icon !== '' ) {
					...
					$social_icons[] = struktur_select_execute_shortcode( 'qodef_icon', $team_icon_params );
				}
			}
		}
